...
class Application_Form_Decorator_Composite extends Zend_Form_Decorator_Abstract implements Zend_Form_Decorator_Interface {
    ...
    public function buildLabel() {
        ...
        if ($element->isRequired()) {
            $labelTag = sprintf('<label id="label-%s" class="required" for="form-element-%s">%s:'
                    . '<span class="asterisk">*</span></label>', $belongsToPrefix . $element->getName(), $belongsToPrefix . $element->getName(), $label);
        } else {
            $labelTag = sprintf('<label id="label-%s" for="form-element-%s">%s:</label>', $belongsToPrefix . $element->getName(), $belongsToPrefix . $element->getName(), $label);
        }
    ...
    public function buildInput() {
        ...
        return $element->getView()->$helper(
                        $name, $value, $attributes, $element->options
        );
    }
    ...
    public function buildContainer($part = 'all', $content = null) {
        ...
        if ('all' == $part || 'before' == $part) {
            $output .= sprintf('<div id="form-item-%s" class="form-item%s" title="%s">', 
